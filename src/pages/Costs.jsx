              ...
              <tbody className="divide-y divide-border">
                {(() => {
                  const userCosts = users.map(u => {
                    ...
                    const totalCost = uBookings.reduce((s, b) => s + (b.total_cost || 0), 0) + uUsages.reduce((s, mu) => s + (mu.total_price || 0), 0);
                    const openCost = uBookings.filter(b => !b.paid).reduce((s, b) => s + (b.total_cost || 0), 0) + uUsages.filter(mu => !mu.paid).reduce((s, mu) => s + (mu.total_price || 0), 0);
                    return { ...u, userBookings: uBookings, userUsages: uUsages, totalCost, openCost };
                  });
                  const sorted = [...userCosts].sort((a, b) => {
                    let aVal = a[sortBy === "name" ? "full_name" : sortBy === "openCost" ? "openCost" : "totalCost"];
                    let bVal = b[sortBy === "name" ? "full_name" : sortBy === "openCost" ? "openCost" : "totalCost"];
                    if (aVal == null) aVal = "";
                    if (bVal == null) bVal = "";
                  ...
                  return sorted.map(u => {
                    ...
                    return (
                      <tr key={u.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-destructive">{u.openCost.toFixed(2)} €</td>
                        <td className="px-4 py-3 text-right font-semibold">{u.totalCost.toFixed(2)} €</td>
                      </tr>
                    );
                  });
                })()}
