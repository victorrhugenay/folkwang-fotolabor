    ...
    <div className={`relative ${className}`}>
      {value ? (
        <div className="relative group w-full h-36 rounded-lg overflow-hidden border border-border">
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <button
              ...
              disabled={uploading}
              className="bg-white text-black rounded-md px-3 py-1.5 text-xs font-medium hover:bg-gray-100 disabled:opacity-50"
            >
              Ändern
            </button>
            <button
        ...
        <button
          ...
          onClick={() => inputRef.current?.click()}
          disabled={uploading}
          className="w-full h-36 rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 text-muted-foreground hover:border-primary hover:text-primary transition-colors disabled:opacity-50"
        >
          {uploading ? (
