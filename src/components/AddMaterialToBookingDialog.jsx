  ...
  useEffect(() => {
    if (open && currentUser?.email) {
      base44.entities.Booking.filter({ status: "confirmed" }).then(all => {
        ...
        setBookings(mine);
        setSelectedBookingId(mine[0]?.id || "none");
      });
    }
  }, [open, currentUser]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4 py-2">
          ...
          <div>
            <Label>Buchung zuordnen (optional)</Label>
            <Select value={selectedBookingId} onValueChange={setSelectedBookingId}>
              <SelectTrigger><SelectValue placeholder="Keine Buchung" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Ohne Buchung</SelectItem>
                {bookings.map(b => (
